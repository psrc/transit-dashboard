...
transit_mode_server <- function(id) {
  moduleServer(id, function(input, output, session){
    ...
    filtered_boardings_df <- reactive({
      ntd_data |> 
        filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & grouping == period_boardings())
    })
    
    filtered_hours_df <- reactive({
      ntd_data |> 
        filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & grouping == period_hours())
    })
    
    filtered_miles_df <- reactive({
      ntd_data |> 
        filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & grouping == period_miles())
    })
    
    filtered_bph_df <- reactive({
      ntd_data |> 
        filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & grouping == period_bph())
    })
    
    # Section Titles
    output$boardings_chart_title <- renderText(paste0("Regionwide ", mode_metric(), " Boardings"))
    output$hours_chart_title <- renderText(paste0("Regionwide ", mode_metric(), " Revenue-Hours"))
    output$miles_chart_title <- renderText(paste0("Regionwide ",mode_metric(), " Revenue-Miles"))
    output$bph_chart_title <- renderText(paste0("Regionwide ",mode_metric(), " Boardings per Hour"))
     
    # Insights
    output$insights_boardings_text <- renderUI({HTML(page_information(tbl=page_text, page_name="Transit", page_section = "Mode-Boardings", page_info = "description"))})
    output$insights_hours_text <- renderUI({HTML(page_information(tbl=page_text, page_name="Transit", page_section = "Mode-Hours", page_info = "description"))})
    output$insights_miles_text <- renderUI({HTML(page_information(tbl=page_text, page_name="Transit", page_section = "Mode-Miles", page_info = "description"))})
    output$insights_bph_text <- renderUI({HTML(page_information(tbl=page_text, page_name="Transit", page_section = "Mode-BPH", page_info = "description"))})
    ...
    output$pre_pandemic_boardings_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull())/1000000, 1), "M")})
    output$current_boardings_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull())/1000000, 1), "M")})
    output$pandemic_share_boardings <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull()), 3)*100, "%"))
    output$recent_share_boardings <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings" & year == base_yr & grouping != "Annual") |> select("estimate") |> pull())*100-100, 1), "%"))
    ...
    output$pre_pandemic_hours_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull())/1000000, 1), "M")})
    output$current_hours_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull())/1000000, 1), "M")})
    output$pandemic_share_hours <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull()), 3)*100, "%"))
    output$recent_share_hours <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Hours" & year == base_yr & grouping != "Annual") |> select("estimate") |> pull())*100-100, 1), "%"))
    ...
    output$pre_pandemic_miles_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull())/1000000, 1), "M")})
    output$current_miles_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull())/1000000, 1), "M")})
    output$pandemic_share_miles <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull()), 3)*100, "%"))
    output$recent_share_miles <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Revenue-Miles" & year == base_yr & grouping != "Annual") |> select("estimate") |> pull())*100-100, 1), "%"))
    ...
    output$pre_pandemic_bph_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull()), 1))})
    output$current_bph_value <- renderText({paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()), 1))})
    output$pandemic_share_bph <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & year == pre_pandemic & grouping != "Annual") |> select("estimate") |> pull()), 3)*100, "%"))
    output$recent_share_bph <- renderText(paste0(round((ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & year == as.character(year(Sys.Date())) & grouping != "Annual") |> select("estimate") |> pull()) / (ntd_data |> filter(variable == mode_metric() & geography == "Region" & metric == "Boardings-per-Hour" & year == base_yr & grouping != "Annual") |> select("estimate") |> pull())*100-100, 1), "%"))
