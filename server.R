shinyServer(function(input, output) {
  ...
  # Region Page
  output$region_page_title <- renderText({
    req(input$RegionMetric)
    paste0(input$RegionMetric, " for all Transit Modes")
    })
  
  output$region_chart_title <- renderText({
    req(input$RegionMetric)
    paste0("Regional ",input$RegionMetric)
    })
  ...
  # Boardings
  output$operator_boardings_chart_title <- renderText(paste0(input$NTDoperators, " Boardings"))
  ...
  # Revenue Hours
  output$operator_hours_chart_title <- renderText(paste0(input$NTDoperators, " Revenue-Hours"))
  ...
  # Revenue Miles
  output$operator_miles_chart_title <- renderText(paste0(input$NTDoperators, " Revenue-Miles"))
  ...
  # Boardings per Hour
  output$operator_bph_chart_title <- renderText(paste0(input$NTDoperators, " Boardings per Hour"))
